...
library(kableExtra)
table_part1 <- data %>%
  dplyr::select(statut_cas_temoin, age, sexe, region, imc, cholesterol_total) %>% 
  tbl_summary(
    by = statut_cas_temoin,
    statistic = list(all_continuous() ~ "{mean} ({sd})", 
                     all_categorical() ~ "{n} ({p}%)"),
    digits = all_continuous() ~ 1,
    label = list(age ~ "Âge", imc ~ "IMC", cholesterol_total ~ "Cholestérol")
  ) %>%
  add_p() %>%
  add_overall() %>%
  bold_labels() %>%
  modify_caption("Caractéristiques Démographiques") %>%
  ...
  # --- OPTIONS KABLE EXTRA ---
  kable_styling(
    latex_options = c("HOLD_position", "striped"), # HOLD_position bloque le tableau ICI
    font_size = 8,                                 # Taille de police réduite
    full_width = FALSE
  )
...
```{r, echo=FALSE}
# --- Tableau 1b : Habitudes de Vie et Antécédents ---
table_part2 <- data %>%
  dplyr::select(statut_cas_temoin, tabagisme, hypertension, diabete, 
  ...
  tbl_summary(
    ...
    statistic = list(all_continuous() ~ "{mean} ({sd})", 
                     all_categorical() ~ "{n} ({p}%)"),
    digits = all_continuous() ~ 1,
    label = list(tabagisme ~ "Tabagisme", 
  ...
  add_overall() %>%
  bold_labels() %>%
  modify_caption("Habitudes de vie et Antécédents") %>%
  
  # --- CONVERSION ET STYLE IDENTIQUES ---
  as_kable() %>%  
  kable_styling(
    latex_options = c("HOLD_position", "striped"), # Bloque la position
    font_size = 8,                                 # Même taille que le tab 1
    full_width = FALSE
  )
...
**Mode de vie et comportements** Le groupe des cas présente des habitudes de vie plus à risque ($p < 0,001$) : 
