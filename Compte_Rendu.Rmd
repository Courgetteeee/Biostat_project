...
où F est la fonction de répartition de la loi logistique. De plus, dans le modèle logistique, ona :
...
```{r, echo=FALSE}
# Conversion du premier tableau
table_clinique_data <- data %>%
  dplyr::select(statut_cas_temoin, age, sexe, imc, cholesterol_total, hypertension, diabete) %>% 
  tbl_summary(
    by = statut_cas_temoin,
    statistic = list(all_continuous() ~ "{mean} ({sd})", all_categorical() ~ "{n} ({p}%)"),
    digits = all_continuous() ~ 1,
    label = list(age ~ "Âge (années)", imc ~ "IMC (kg/m²)", cholesterol_total ~ "Cholestérol (mmol/L)")
  ) %>%
  add_p() %>%
  add_overall() %>%
  as_tibble() # Conversion pour kable

knitr::kable(
  table_clinique_data,
  col.names = c("Variable", "Total (N=520)", "Cas (N=260)", "Témoin (N=260)", "p-value"),
  caption = "Caractéristiques cliniques et démographiques",
  booktabs = TRUE, 
  align = c("l", "c", "c", "c", "c")
) %>%
...
```{r, echo=FALSE}
table_vie_data <- data %>%
  dplyr::select(statut_cas_temoin, tabagisme, niveau_activite_physique, consommation_alcool) %>% 
  tbl_summary(
    by = statut_cas_temoin,
    statistic = list(all_categorical() ~ "{n} ({p}%)"),
    label = list(tabagisme ~ "Tabagisme", niveau_activite_physique ~ "Activité physique", consommation_alcool ~ "Alcool")
  ) %>%
  add_p() %>%
  add_overall() %>%
  as_tibble()
...
knitr::kable(
  table_vie_data,
  col.names = c("Variable", "Total (N=520)", "Cas (N=260)", "Témoin (N=260)", "p-value"),
  caption = "Habitudes de vie et comportements à risque",
  booktabs = TRUE,
  align = c("l", "c", "c", "c", "c")
) %>%
  kableExtra::kable_styling(
  ...
  kableExtra::column_spec(1, bold = TRUE) %>%
  kableExtra::add_header_above(c(" " = 1, "Répartition par statut" = 3, " " = 1))
```
...
Avant de commencer nos analyses et nos modèles de régressions logistiques, on va changer les variables de références pour nos variables qualitatives.
...
Dans cette partie, nous allons tester de faire des `glm` simple pour chacune de nos variables avec le code suivant :
...
Dans la suite nous afficherons plus le code.
...
On obtient $\hat{\beta}_{age} = 0.083$ qui estime $\beta_{age} = \ln\left(\frac{odds(Y=1 \mid age=x+1)}{odds(Y=1 \mid age=x)}\right)$.

L'intervalle de confiance à 95% de l'odds ratio est : $\exp(\beta_{age}) \in [1.06; 1.11]$.

Cela signifie que pour chaque année d'âge supplémentaire, l'odds de faire un infarctus est multiplié par au moins **1.06** et au plus **1.11**. Autrement dit, le risque augmente d'environ 6% à 11% par an.

Comme l'intervalle ne contient pas la valeur 1 (borne inférieure \> 1) et que la p-value est extrêmement faible ($p < 0.001$), l'âge est un **facteur de risque statistiquement significatif**.
...
On obtient $\hat{\beta}_{chol} = 0.785$ qui estime $\beta_{chol}$.

L'intervalle de confiance à 95% de l'odds ratio est : $\exp(\beta_{chol}) \in [1.79; 2.71]$.

L'association est très forte : pour une augmentation d'une unité de cholestérol total, l'odds d'être un cas est multiplié par environ **2.19** (l'estimateur ponctuel $e^{0.785}$), avec un intervalle allant de 1.79 à 2.71.

La p-value est très inférieure au seuil de 0.05, confirmant que l'hypercholestérolémie est un **facteur de risque majeur et significatif** dans cet échantillon.
...
On obtient $\hat{\beta}_{imc} = 0.148$ qui estime $\beta_{imc}$.

L'intervalle de confiance à 95% de l'odds ratio est : $\exp(\beta_{imc}) \in [1.10; 1.22]$.

Ainsi, une augmentation de 1 point d'IMC (par exemple passer de 24 à 25 kg/m²) multiplie l'odds de maladie par un facteur compris entre **1.10** et **1.22**.

La p-value étant très faible ($6.7 \times 10^{-10}$), l'IMC est **significativement associé** à la maladie : plus l'IMC est élevé, plus le risque augmente.
...
-   On obtient : $\hat{\beta}_{ancien \space fumeur}$=0.407 qui estime $\beta_{ancien \space fumeur} = ln(\frac{odds(Y=1 \space | \space tabagisme =\text{ancien fumeur, } X_{-tabac} \text{ fixé})}{odds(Y=1 \space | \space tabagisme = Jamais, \space X_{-tabac} \text{ fixé})})$. L'intervalle de confiance à 95% de l'odds ratio est : $exp(\beta_{ancien \space fumeur})\in [e^{-0.13} ; e^{0.95}] = [0.88;2.59]$.

    En analyse univariée, l'effet était significatif (p = 0.024). Après ajustement sur les autres facteurs de risque (âge, IMC, hypertension, diabète, cholestérol, activité physique, consommation d'alcool), cet effet n'est plus significatif. Cela suggère que l'association observée en univarié était en partie due à des facteurs confondants.)
...
-   On obtient : $\hat{\beta}_{fumeur \space actuel} =0.995 \text{ qui estime } \beta_{fumeur \space actuel} = ln(\frac{odds(Y=1 \space | \space tabagisme =\text{Fumeur actuel, }X_{-tabac} \text{ fixé})}{odds(Y=1 \space | \space tabagisme = Jamais, \space X_{-tabac} \text{ fixé})})$. L'intervalle de confiance à 95% de l'odds ratio est : $exp(\beta_{fumeur \space actuel})\in [e^{0.48} ; e^{1.52}] = [1.62;4.57]$.
    ...
    En passant au modèle multivarié, le tabagisme actif reste significatif, mais la diminution de l'odds ration (IC et estimation) peut supposer qu'une partie de l'effet brut était dû à d'autres facteurs....
...
knitr::kable(interactions_summary, 
             caption = "Résultats des tests d'interactions (test Anova)")
```
...
L'abscence d'interactions significatives justifie donc le choix du modèle additif retenu précèdemment.

Cependant, l'abscence d'interactions dans notre échantillon (n=520) ne signifie pas qu'elles n'existent pas dans la population générale.
...
Des analyses analogues ont été réalisées sur la consommation d'alcool et le niveau d'activité phyisique, mais aussi la réalisation de modèle polytomique non ordonnée (variable Sexe et Region). Cependant cette approche s'est rélévée peu informative dans notre contexte.

\appendix

# TODO

-   Commenter les odds du modele total ?
-   Quelles sorties + quels morceaux de code on affiche ?
